...
try {
    ...
    $rabbitPlugins = Get-ChildItem "C:\Program Files\RabbitMQ Server" -Name "rabbitmq_server-*" | Select-Object -First 1
    if ($rabbitPlugins) {
        $pluginPath = "C:\Program Files\RabbitMQ Server\$rabbitPlugins\sbin\rabbitmq-plugins.bat"
        & $pluginPath enable rabbitmq_management
    }
